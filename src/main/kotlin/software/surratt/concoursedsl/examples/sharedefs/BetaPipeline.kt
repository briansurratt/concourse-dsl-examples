 ...
 *
 */
private val BetaSourceCode = Resource(
        "blpha-sourcecode",
        "git",
        source = mapOf(
                "uri" to "git@github.com:exampleproject/beta.git",
                "branch" to "master",
 ...
 *
 */
val BetaPipeline = Pipeline().apply {
    ...
    job("dev") {
        plan {
            ...
            get(BetaSourceCode.name) {
                trigger = true
            }
            task("build") {
                file = "pipeline/tasks/build.yml"
                inputMapping = mapOf(
                        "source-code" to BetaSourceCode.name,
...
fun main(args: Array<String>) {
    println(generateYML(BetaPipeline))
}
